...
Ansible은 인프라 자동화, 구성 관리, 애플리케이션 배포를 위한 강력한 오픈소스 도구입니다:

- 🤖 **자동화**: 서버 설정, 배포, 오케스트레이션 자동화
- 📜 **YAML 문법**: 읽기 쉬운 Playbook으로 인프라 코드화
- 🐧 **Alpine Linux**: 경량 Alpine 3.20 기반 (이미지 크기 최소화)
- 🔧 **Ansible 2.18**: 최신 Ansible 버전
- 🌐 **에이전트리스**: SSH만으로 원격 서버 관리
- 📦 **필수 도구**: bash, curl, git, openssh-client, sshpass
- ☁️ **AWS 지원**: boto3 라이브러리로 클라우드 리소스 관리
- ⚡ **최적화**: SSH pipelining, 스마트 팩트 수집
- 🔐 **보안**: SSH 키 기반 인증 지원
- 📚 **Galaxy 연동**: Ansible Galaxy roles 설치 가능
