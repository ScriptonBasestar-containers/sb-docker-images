...
Ethereum Geth (Go Ethereum) 풀 노드와 BlockScout 블록 탐색기를 Docker로 실행할 수 있는 환경입니다:

- ⚡ **Geth 노드**: 공식 Ethereum Go 클라이언트
- 🔍 **BlockScout**: Ethereum 블록체인 탐색기
- 🌐 **다중 네트워크**: Mainnet, Sepolia, Goerli, Dev Mode 지원
- 🔐 **JSON-RPC API**: HTTP 및 WebSocket 인터페이스
- 💾 **데이터 영속성**: Docker 볼륨으로 블록체인 데이터 보존
- 📊 **동기화 모드**: Snap, Full, Light 동기화 지원
- 🔒 **보안**: API 제한 및 네트워크 제어 설정
