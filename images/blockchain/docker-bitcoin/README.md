...
Bitcoin Core 풀 노드와 BTC RPC Explorer를 Docker로 실행할 수 있는 환경입니다:

- 🪙 **Bitcoin Core**: 완전한 Bitcoin 풀 노드 실행
- 🔍 **BTC Explorer**: 블록체인 탐색 웹 인터페이스
- 🌐 **다중 네트워크**: Mainnet, Testnet, Regtest 지원
- 🔐 **RPC API**: JSON-RPC 인터페이스 제공
- 💾 **데이터 영속성**: Docker 볼륨으로 블록체인 데이터 보존
- 📊 **모니터링**: 동기화 상태 및 네트워크 통계 확인
- 🔒 **보안**: RPC 인증 및 네트워크 제어 설정
